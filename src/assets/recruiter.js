...
const Recruiter = ()=>{
    ...
    return(
        <div className="">
        <div className="">
          {MemberCarouselData.map((img, key) => {
            return (
              <div key={key} className={`${mobileSwiperContainer({ index: memberCarouselIndex })}`}>
              </div>
            );
          })}
        </div>
              ...
              <div className={mobileSwiperContainer({ index: memberCarouselIndex })} key={key}>
                <div className="mobileCarouselItem">
                  <div className={`${iconCont} icon ${memberCarouselIndex === key ? 'active' : ''}`}>
                    <span className="no">{key + 1}</span>
                    <img src={member.img} />
                  </div>
                  <div className="">
                    <div className="">{member.heading}</div>
